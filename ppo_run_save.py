...
from openai_modified import sigmoid_policy
import gym
import gym_target
import argparse
...
def train(num_timesteps, save_prefix, restore_file, task, policy, graph=True):
	U.make_session(num_cpu=1).__enter__()
	def policy_fn(name, ob_space, ac_space):
		if policy == "sigmoid":
			return sigmoid_policy.SigmoidPolicy(name = name, ob_space=ob_space, ac_space=ac_space,
			hid_size = 32, num_hid_layers=3)
		elif policy == "mlp":
			return mlp_policy.MlpPolicy(name=name, ob_space=ob_space, ac_space=ac_space,
			hid_size = 32, num_hid_layers = 3)
		elif policy == "beta":
			return beta_policy.BetaPolicy(name = name, ob_space=ob_space, ac_space=ac_space,
			hid_size = 32, num_hid_layers=3)
...
def main():
	...
	parser.add_argument("-p", "--policy", type=str, help="mlp, sigmoid, beta")
	parser.add_argument("-g", "--graph", type=bool)
	args = parser.parse_args()
	train(args.num_timesteps, args.save_prefix, args.restore_file, args.task, args.policy, args.graph)
