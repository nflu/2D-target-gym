...
class TargetDynamicsEnv(gym.Env):
	...
	def __init__(self):
		...
		#update 2: this can be fixed using beta distribution implemented here https://github.com/brendenpetersen/baselines/blob/master/baselines/ppo2/policies.py
		...
		self.observation_space = spaces.Box(low = -self.high[:self.stateDim], high = self.high[:self.stateDim], dtype = np.float32) #observation is just state
		self.curr_step = 0
	...
	def reset(self):
		self.targetPoint = np.zeros(2) #the target point is fixed in space over all episodes 
		def l(point):
	...
	def _take_action(self, action):
		a = np.clip(action, -self.high[-2:], self.high[-2:])
		self.state[0] = self.state[0] + self.timestep_size * self.state[2]
		self.state[1] = self.state[1] + self.timestep_size * self.state[3]
		self.state[2] = self.state[2] + self.timestep_size * a[0]
		self.state[3] = self.state[3] + self.timestep_size * a[1]
	...
	def _render(self, mode="human", close=False):
		...
		# Fix: The fix is to use _close() before close() is called  
